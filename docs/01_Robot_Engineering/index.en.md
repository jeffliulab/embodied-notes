# Robot Engineering

Robot Engineering focuses on the **engineering practice** of each robot subsystem — from computer architecture to sensors, actuators, communication, power, and mechanical structures. This section takes the perspective of hardware engineers and makers, diving deep into working principles, interface protocols, representative product teardowns, and practical programming examples.

Using humanoid robots, quadrupeds, robot vacuums, and robot arms as primary references, ensuring coverage of all critical engineering subsystems.

**Contents:**

- **[Computer Engineering](01_Computer_Engineering/index.md)** — CPU/GPU architecture, memory, buses, embedded systems, OS
- **[Cameras & Vision](02_Cameras/index.md)** — RGB/depth/stereo/event cameras, calibration
- **[LiDAR](03_LiDAR/index.md)** — 2D/3D/solid-state LiDAR, point cloud processing
- **[Inertial Navigation & Localization](04_IMU_Navigation/index.md)** — IMU, VIO, GPS/RTK, proximity sensors
- **[Motors & Actuators](05_Motors_Actuators/index.md)** — DC/BLDC/servos, gearboxes, end effectors
- **[Communication & Bus](06_Communication/index.md)** — UART/I2C/SPI/CAN/EtherCAT/WiFi
- **[Power Management](07_Power/index.md)** — Batteries, BMS, DC-DC, power budget
- **[Mechanical Structure](08_Mechanical/index.md)** — Chassis, arms, thermal/IP protection, 3D printing, CAD
- **[Displays & HMI](09_Displays_HMI/index.md)** — OLED/LCD, web interface, remote control
- **[Force & Tactile Sensing](10_Force_Tactile/index.md)** — 6-axis F/T, tactile skin, foot sensors, force control
- **[System Integration](11_Integration/index.md)** — PCB design, ROS2 integration, debugging, robot teardowns
