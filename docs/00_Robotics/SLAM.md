...
LiDAR SLAM 使用激光雷达获取精确的 3D 点云（参见 [传感器](../07_Hardware/传感器.md)），适合大规模室外环境。
...
SLAM 系统通常运行在 ROS/ROS2 框架下（参见 [ROS2](../06_Software/ROS2.md)），与导航栈协同：

- SLAM 提供位姿估计和地图
- 导航栈基于地图进行路径规划
- 控制器执行运动命令

传感器的选型和标定对 SLAM 性能至关重要（参见 [传感器](../07_Hardware/传感器.md)）。
