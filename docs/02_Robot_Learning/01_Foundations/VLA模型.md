...
> 相关笔记：[模型发展路线图](模型发展路线图.md) | [ACT模型](../02_Methods/ACT模型.md) | [模仿学习](../02_Methods/模仿学习.md) | [扩散策略](../02_Methods/扩散策略.md) | [机器人基础模型概论](机器人基础模型概论.md) | [开源模型汇总](../03_Models/开源模型汇总.md)
...
> 更多关于扩散策略的内容参见：[扩散策略](../02_Methods/扩散策略.md)
...
这条设计路线在具身智能里并不是凭空出现的。它在模型谱系上的关键桥接节点是 [ACT模型](../02_Methods/ACT模型.md)：ACT 把 chunked action prediction 清晰地变成了一个可复现、可解释的策略范式，后续很多 VLA 的 horizon 设计、动作块推理和时间平滑都能在那条线上找到动机。
