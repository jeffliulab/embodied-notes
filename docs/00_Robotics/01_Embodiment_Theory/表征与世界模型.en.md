...
- $q(s|o)$: Approximate posterior (brain's approximate posterior belief over hidden states)
- $p(s|o)$: True posterior
- $p(o)$: Log-likelihood of observations (model evidence)

Since $D_{KL}[q(s|o) \| p(s|o)] \geq 0$, we have $F \geq -\ln p(o)$; that is, free energy is an upper bound on the negative log model evidence.

**Equivalent decomposition (complexity − accuracy form)**:

$$F = \underbrace{D_{KL}[q(s) \| p(s)]}_{\text{complexity}} - \underbrace{\mathbb{E}_{q}[\ln p(o|s)]}_{\text{accuracy}}$$

where $p(s)$ is the prior belief and $p(o|s)$ is the likelihood of the generative model. Minimizing free energy is equivalent to:
...
- [World Models and Video Generation](../../02_Robot_Learning/01_Foundations/世界模型与视频生成.md)
