...
In [Virtual World Simulation Engines](../../05_AI_Agents/07_Virtual_Embodied_Agents/虚拟世界仿真引擎.en.md), the Smallville example looks like social simulation, not robotics. But it demonstrates an important engineering idea: a world is not just an image or a mesh. It is a semantic tree.
...
This section does not re-derive dynamics from first principles. For that, see [Dynamics](../03_Robotics/动力学.en.md). Here the focus is how a simulator closes the minimum dynamics loop needed for a useful world.
...
Support varies strongly by format and engine, which is why [Development Toolchain](开发工具链.en.md) and simulator choice matter upstream of task design.
...
For broader transfer strategy, see [Sim2Real](../04_Robot_Learning/Sim2Real.en.md).
...
- For URDF, MJCF, SDF, USD, and surrounding tooling, see [Development Toolchain](开发工具链.en.md).
- For transfer strategy and domain randomization principles, see [Sim2Real](../04_Robot_Learning/Sim2Real.en.md).
- For the robot-side control abstractions that world rules ultimately serve, see [Control Theory](../03_Robotics/控制理论.en.md).
