...
ACT is the representative bridge from classic imitation learning to chunk-based policies. This page keeps it as a method entry; for the fuller model-level explanation, read [ACT Model](../05_Models/ACT模型.en.md).
...
If you want the full story of how ACT influenced later VLAs and action tokenization work, continue with [ACT Model](../05_Models/ACT模型.en.md) and [Model Roadmap](../05_Models/模型发展路线图.en.md).
...
2. **VLA Models**: Combining vision-language foundation models with action prediction, see [VLA Models](../05_Models/VLA模型.md)
3. **Data Scaling**: Large-scale data collection through better teleoperation systems, see [Teleoperation and Data Collection](遥操作与数据收集.md)
