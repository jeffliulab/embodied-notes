...
ACT 是模仿学习走向 chunk-based policy 的代表模型。这里保留它作为方法入口，但更完整的模型级解释请直接看 [ACT模型](../05_Models/ACT模型.md)。
...
如果你想进一步理解它为什么会影响后来的 VLA 和 action tokenization 设计，请继续读 [ACT模型](../05_Models/ACT模型.md) 和 [模型发展路线图](../05_Models/模型发展路线图.md)。
...
2. **VLA 模型**：将视觉-语言大模型与动作预测结合，详见 [VLA模型](../05_Models/VLA模型.md)
3. **数据规模化**：通过更好的遥操作系统大规模收集数据，详见 [遥操作与数据收集](遥操作与数据收集.md)
