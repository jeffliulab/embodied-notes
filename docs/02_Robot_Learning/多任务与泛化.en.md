...
- **Foundation models**: [Models and Algorithms](../05_Models/index.md) — VLA and world models are key to achieving zero-shot generalization
- **Imitation learning**: [Imitation Learning](模仿学习.md) — BC and diffusion policies are the underlying algorithms for multi-task learning
- **Diffusion policy**: [Diffusion Policy](扩散策略.md) — its multimodal modeling capability is particularly important for multi-task learning
- **Data collection**: [Teleoperation and Data Collection](遥操作与数据收集.md) — data scaling directly affects generalization capability
