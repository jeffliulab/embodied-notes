...
### The Core Teardown Insight: Gear Ratio vs Torque Transparency

The deepest engineering divergence among the four robots is not in sensors or the main controller, but in the **joint transmission scheme**. Plotting each robot's joint on a "gear ratio (log x-axis) — torque transparency / back-drivability (y-axis)" plane immediately reveals a clear inverse-correlation curve: the higher the gear ratio, the higher the output torque density, but the harder it is to back-drive and the more an extra joint-torque sensor is needed for force control. This is exactly the engineering trade-off line that the later "Cross-Product Comparison" table keeps circling around.

<!-- SVG-DESIGN-NOTES
Type: D (quantitative relation / scale — gear-ratio vs torque-transparency trade-off scatter)
Q0 (One Big Idea): joint gear ratio strongly inversely correlates with torque transparency — QDD (6-9:1) is back-drivable and estimates force from current, harmonic (100:1) is nearly non-back-drivable and must add a joint-torque sensor; this curve explains why Panda needs a built-in torque sensor while G1/Go2 do not
Q1 (geometric mapping): semi-log scatter — X = gear ratio N (log, 1..200), Y = torque transparency / back-drivability (0..1 qualitative axis); the four robots' joints as points placed by real N (Go2≈9, Roomba no reduction≈1, Panda 100, G1≈7), overlaid with a monotone-decreasing trade-off trend curve
Q2 (DNA): drop the title — a curve falling monotonically from upper-left (direct-drive transparent) to lower-right (high-ratio opaque) + four real product points scattered along it; this is the standard transmission-design trade-off chart, impossible to confuse with a system block diagram
Q3 (decoration removed): deleted the original 15+ equal-size component boxes + 14 crossing arrows (a system block diagram unrelated to the "teardown insight," pure filler); no legend, product names sit directly next to the points
Q4 (direct labels): each robot name + gear ratio labeled right at the data point; axis ends marked with log ticks 1/10/100; trend curve labeled "needs an independent torque sensor ↑"
Q5 (Dark + bilingual): all var(--dia-*); QDD-type in green (transparency advantage), harmonic in accent (opaque), trend line in stroke-soft; Chinese product labels swapped in the EN version
-->
<div class="diagram">
<svg viewBox="0 0 720 400" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Gear-ratio vs torque-transparency trade-off of four robots' joints">
  <text x="360" y="28" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-style="italic" font-weight="600" font-size="17" fill="var(--dia-stroke)">Joint Transmission — Gear Ratio vs Torque Transparency</text>
  ...
  <text x="80" y="78" text-anchor="end" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="11" fill="var(--dia-stroke-soft)">high (back-drivable)</text>
  <text x="80" y="204" text-anchor="end" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="11" fill="var(--dia-stroke-soft)">mid</text>
  <text x="80" y="328" text-anchor="end" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="11" fill="var(--dia-stroke-soft)">low (needs torque sensor)</text>
  <text x="34" y="200" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="12" fill="var(--dia-stroke)" transform="rotate(-90 34 200)">Torque transparency / back-drivability</text>
  ...
  <text x="370" y="372" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="12" fill="var(--dia-stroke)">Gear ratio N  (log)</text>
  ...
  <text x="500" y="178" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="11" fill="var(--dia-stroke-soft)">ratio ↑ ⇒ transparency ↓ ⇒ needs independent torque sensor</text>

  <circle cx="98" cy="160" r="6" fill="var(--dia-green)"/>
  <text x="112" y="150" text-anchor="start" font-family="Fraunces, Georgia, serif" font-size="11" fill="var(--dia-green)">Roomba j7 drive motor (DC, N≈1)</text>
  ...
  <text x="322" y="156" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-size="11" fill="var(--dia-green)">Unitree Go2 planetary (N≈9)</text>

  <circle cx="578" cy="300" r="8" fill="var(--dia-accent)"/>
  <text x="578" y="288" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-weight="600" font-size="11" fill="var(--dia-accent)">Franka Panda harmonic (N=100)</text>
  <path d="M 578 308 C 560 318 540 320 520 320" stroke="var(--dia-accent)" stroke-width="1" fill="none" stroke-dasharray="3 3"/>
  <text x="515" y="324" text-anchor="end" font-family="Fraunces, Georgia, serif" font-style="italic" font-size="10" fill="var(--dia-accent)">∴ built-in strain-gauge torque sensor</text>
</svg>
</div>
<p class="figure-caption">Figure 1 — The joints of all four torn-down robots fall on the same trade-off curve: QDD (G1) and low-ratio planetary (Go2) sit upper-left, back-drivable and estimating force from motor current; harmonic reduction (Panda, 100:1) sits lower-right, nearly non-back-drivable, which is why a torque sensor must be built into every joint. This curve is the geometric backbone of the later cross-product comparison.</p>
